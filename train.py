...
""" Assigning Height and width parameters """
H = 512
W = 512

#checking if the specified path exists or not, if not then creat the one.
def create_dir(path):
    """ Create a directory. """
...
# Seeding is used make stable randomness, if i run it again same randomness will be there.
...
lr = 1e-5
num_epochs = 16
model_path = os.path.join("files", "model.h5")  #Saving in h5
csv_path = os.path.join("files", "data.csv")    #Saving CSV
...
'''
#train_x= X-ray image, train_y1= mask1, train_y2=mask2

'''
x = read_mask(train_y1[0], train_y2[0])
print(x.shape)
cv2.imwrite("im2.png", x*255)
'''
